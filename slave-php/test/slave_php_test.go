...
var _ = BeforeSuite(func() {
	...
	if imageName == "" {
		imageName = "openshift/jenkins-slave-php-centos7-candidate"
	}
})

var _ = Describe("php Slave testing", func() {
	var id string
	...
	It("should contain a runnable php", func() {
		...
		id, err = dockercli.ContainerCreate(
			&container.Config{
				Image:      imageName,
				Entrypoint: []string{"/bin/bash", "-c", "php --version"},
				Tty:        true,
			},
			nil)
		Expect(err).NotTo(HaveOccurred())
